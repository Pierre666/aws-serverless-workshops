...
In this module you'll create an [Amazon Cognito](https://aws.amazon.com/cognito/details/) user pool to manage your users' accounts. You'll deploy pages that enable customers to register as a new user, verify their email address, and sign into the site.
...
After users have a confirmed account (either using the email verification process or a manual confirmation through the console), they will be able to sign in. When users sign in, they enter their username (or email) and password. A JavaScript function then communicates with Amazon Cognito, authenticates using the Secure Remote Password protocol (SRP), and receives back a set of [JSON Web Tokens](https://jwt.io/) (JWT). The JWTs contain claims about the identity of the user and will be used in the next module to authenticate against the [REST](https://en.wikipedia.org/wiki/Representational_state_transfer)ful API you build with [Amazon API Gateway](https://aws.amazon.com/api-gateway/features/).
